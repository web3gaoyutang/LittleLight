...
完整技术文档见：`docs/engineering-technical-design.md`。机器可读 API 契约见：`docs/openapi.yaml`。数据库说明见：`docs/database-schema.md`。部署运行手册见：`docs/deployment-runbook.md`。工程验证矩阵见：`docs/engineering-verification-matrix.md`。
...
- `.github/workflows/engineering-checks.yml` 包含 OpenAPI、Docker Compose 配置、Go 测试、前端 API client 测试、H5 构建和 Web/API Docker 镜像构建检查。
...
```bash
cd server && go test ./...
cd ../app && npm run test:api && npm run build:h5
