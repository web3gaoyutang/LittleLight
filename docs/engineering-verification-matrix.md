...
| 场景 | 命令 | 覆盖范围 |
| --- | --- | --- |
| 本地快速回归 | `powershell -ExecutionPolicy Bypass -File .\scripts\verify-all.ps1` | OpenAPI、环境示例、Compose、Web readiness、文档覆盖、Go 测试、前端 API client 测试、前端页面契约测试、H5 构建、密钥检查 |
...
| API smoke 回归 | `node scripts/api-smoke.mjs` | 运行中 API 的 health/readiness、微信开发登录、首页、课程、待办、家长、沟通、AI、疗愈、收藏链路 |
| H5 smoke 回归 | `cd app && npm run test:h5-smoke` | 运行中 H5 的开发登录、首页跳转、日程页真实待办表单提交、API 落库校验 |
| CI 回归 | `.github/workflows/engineering-checks.yml` | OpenAPI、文档覆盖、Compose、Go 测试、前端 API client 测试、前端页面契约测试、H5 构建、API smoke、H5 smoke、Web/API Docker 镜像构建 |

## 目标到证据映射

| 目标要求 | 当前证据 | 验证方式 | 状态 |
| --- | --- | --- | --- |
| App 使用 uni-app | `app/src`、`app/src/pages.json`、`app/vite.config.js`、`app/package.json` | `npm run build:h5`、`npm run test:api`、`npm run test:pages` | 已实现工程骨架和核心 API 接入 |
...
| 前端 API client 测试 | base URL、鉴权头、微信登录/模拟登录态、上传解析正常 | `app/scripts/test-api-client.mjs` |
| 前端页面契约测试 | 登录页优先、主页面统一状态/鉴权守卫、真实表单 API、导入预览、AI 复核动作和未接入能力边界不回退 | `app/scripts/test-page-contracts.mjs` |
| 服务端输入边界 | JSON body 大小、未知字段、尾随 JSON、业务必填/枚举/长度校验正常 | `server/internal/http/router_test.go` |
| Docker 逻辑验证 | PostgreSQL/Redis/API 主业务链路能本地闭环，并使用登录返回的 Bearer session 而不是 `X-User-ID` 开发鉴权 | `scripts/verify-docker.ps1` |
...
- 生产微信登录需要设置 `APP_ENV=prod` 或 `APP_ENV=production`，配置真实 `WECHAT_APP_ID`、`WECHAT_APP_SECRET` 和足够强的 `SESSION_SECRET`，并关闭 `AUTH_ALLOW_DEV_USER`、`AUTH_ALLOW_MOCK_LOGIN`。
